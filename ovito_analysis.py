...
class Wigner_Seitz:
    ...
    def __init__(self, path, repeats):
        ...
        self.repeat_dirs_path = [tools.Path(path) for path in glob(f"{tools.bombard_directory()}{args_dict['path']}*r")]

        tools.make_dir(f"{tools.bombard_directory()}{path}ovito_results")
    ...
    def publish_results(self):
        ...
        number_with_vancanies = len([x for x in self.results_arr[:,0] if x > 0])
        ...
        number_with_interstitials = len([x for x in self.results_arr[:,1] if x > 0])
        ...
        number_with_etched = len([x for x in self.results_arr[:,2] if x > 0])
