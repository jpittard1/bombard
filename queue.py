...
input_file_obj.edit('region', new_line = f'box block 0 {x_width} 0 {y_width} -55.0 -25.0',second_val='box')
...
for repeat, repeat_input in enumerate(input_file_obj.repeated_inputs):
    ...
    while True: #creating new directory 
        try:
            if multi_bombard == True:
                ...
                new_path = f'{bombard_dir}/results/{bombard_atom}_{energy}eV_{repeats}r_{count}/{repeat}r'

            
            os.mkdir(new_path)
            os.mkdir(f"{new_path}/{repeat}r")
    ...
    shutil.copyfile(f"{bombard_dir}/LAMMPS_files/{data_file_obj.name}", f"{new_path}/{data_file_obj.name}")    
    shutil.copyfile(f"{bombard_dir}/LAMMPS_files/{input_file_name}", f"{new_path}/{input_file_name}")
...
for path in paths:
    ...
    if hpc == True and test == False:         
        shutil.copyfile("LAMMPS_files/srun",f"{path}/srun") #copying srun avoids rewritting
        os.chdir(path)
